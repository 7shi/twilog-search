...
### 整形テキスト表示形式の統一
**Problem**: 従来の出力形式では、search_similarがJSON形式、他のコマンドがYAML形式と表示方法が不統一だった。また、MCPサーバーでもYAML形式を使用していたが、Claude CodeなどのMCPクライアントではYAML文字列の処理が困難で、構造化データとして活用できない問題があった。

**Solution**: 全てのコマンド出力を整形テキスト形式に統一。🔍、📝、👥、📊、🟢などの絵文字を使用して視覚的に見やすく改善。search_similarでは投稿内容の改行保持・空行詰め処理を実装し、reasoning/summary/tagsは削除（LLMクライアントには不要）。数値にはカンマ区切りを適用し、URLは日時後に配置。get_statusコマンドをオーバーライドして、MCPサーバーと同じ整形テキスト形式で表示。これにより、コマンドライン使用時の可読性が向上し、MCPクライアントでの使用体験も改善された。
