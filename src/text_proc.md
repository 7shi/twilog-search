...
### 複合検索（V|T）のパイプライン構文サポート
**Problem**: ベクトル検索とテキスト検索を組み合わせた複合検索や、テキスト検索のみの実行を統一的なインターフェースで扱う必要があった。

**Solution**: パイプライン構文（V|T）を実装し、`parse_pipeline_query`関数でベクトルクエリとテキストクエリを分離。エスケープ処理（\\|）により、パイプ文字を文字通りの意味で使用可能にした。V=""の場合はテキスト検索のみ、T=""の場合はベクトル検索のみを実行する柔軟な設計を採用した。