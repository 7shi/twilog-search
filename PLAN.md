...
- ✅ **タグスコアリング手法比較**: 6手法の性能検証、mean手法（平均順位736.2）の最優秀性確認。詳細は[タグベース検索スコアリング手法比較検証レポート](docs/20250718-tag-scoring.md)を参照
...
- ✅ **類似度vs共起率比較検証**: `tag_similarity_cooc.py`による双方向共起率検証、ベクトル類似度の絶対的優位性確認
...
5. **共起率の検索適用限界**: ベクトル類似度の絶対的優位性（逆転率0.0%）を実証

詳細は[タグベース検索スコアリング手法比較検証レポート](docs/20250718-tag-scoring.md)を参照。
...
**Solution**: 直接的なsummary→全タグ類似度計算によるmean手法を採用継続し、複雑なコアタグ経由のスコアリング手法を設計廃止。LLM（Gemini）によるタグ抽出基準とベクトルモデル（Ruri v3）の表現空間不一致により、中間変換段階が情報損失を招くため。詳細は[タグベース検索スコアリング手法比較検証レポート](docs/20250718-tag-scoring.md)を参照。
...
**結論**: タグは「検索の出発点ではなく、検索結果の整理・分類ツール」として活用すべきであり、現行のハイブリッド検索システム（直接ベクトル検索+タグ補助）が最適解として確定。

詳細な特性分析と活用範囲については[タグベース検索スコアリング手法比較検証レポート](docs/20250718-tag-scoring.md)を参照。
