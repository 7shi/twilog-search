...
- **コマンド**: `/help`でヘルプ、`/user`でユーザーフィルタリング、`/date`で日付フィルタリング、`/top`で表示件数設定、`/mode`で検索モード選択、`/view`で表示モード切り替え、`/details`で指定ランクの詳細表示
...
### 次期実装予定
- タグベース情報整理システム
  - タグ階層分析ツール（包含関係による知識階層構築）
  - カテゴリ分類システム（主要領域の自動分類）
  - 時系列分析ツール（関心変遷の可視化）
  - 知識マップ生成（共起ネットワークによる関連性可視化）
  - 情報整理ダッシュボード（統合表示）

### 研究・実験コード
- `research/`ディレクトリに検索アルゴリズムの実証研究コードを配置
- タグベース検索の限界検証、スコアリング手法比較、最適化技術の検証を実施
- 重要な設計判断（mean手法採用、コアタグ手法廃止等）の科学的根拠を提供
...
- **設定管理**: SearchSettings（ユーザー・日付フィルタリング、表示件数、検索モード、表示モード）、重複除去は常時有効
...
- **settings.py**: 統合設定管理（SearchSettings、シリアライズ対応、表示モード設定）
